...
import math
import sys
from my_classes_LF_ptUnordered_forY import DataGenerator
import pickle
import json
...
with open('upsilon_partition.json') as json_file:
    upsilon_partition = json.load(json_file)
...
validation_generator = DataGenerator(upsilon_partition['upsilon_validation'], **params)
...
pred = big_model.predict_generator(validation_generator, steps = 200) #Batch size 256 * 200 examples


#file=uproot.open("fout_particleGun.root")["tree"] 
#print("GOT TREE FROM fout_particleGun.root")

file = uproot.open("upsilon_fout.root")["tree"]
print("GOT TREE FROM upsilon_fout.root!")
tau_features = []
tau_labels = []
antitau_features = []
antitau_labels = []
...
theDir = dateToInclude + "_" + my_current_time + "_" + "ptUnorderedJanModel_MSE_testOnY"
