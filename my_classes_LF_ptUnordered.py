...
class DataGenerator(Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        # num_of_labels + 1 because num_of_labels here is 2 but the .npy are 3
#NO + 1 in the self.num_of_lables for Y needed now!
        
        #Generate data
        for i, ID in enumerate(list_IDs_temp):
            #Store the features
            X[i,] = np.load('toUse_data_features_unorderedPt/features_example_' + ID + '.npy') 
            #Store labels
            Y[i,] = np.load('toUse_data_labels_unorderedPt/labels_example_' + ID + '.npy')
        
#        Y = Y[:, :2]   #Get just the first 2 columns, since we are only using two labels, pt normalized by tau mass and theta
#We want all 3 labels now, pt tau norm by tau mass, theta, and phi!        
        return X, Y 
